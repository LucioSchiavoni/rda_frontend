...
import { getDocByIdRequest } from '../../api/doc';
...
const DocContent = () => {
  ...
  const { data, isLoading } = useQuery<any, Error>({
    ...
    queryFn: () => getDocByIdRequest(authorId, id || undefined),
  });

  const handleSave = async () => {
    try {
      const res = await clienteAxios.put('/update/doc', {
        authorId,
        id,
      ...
      <div className='flex '>
        ...
        <div className='flex flex-col w-11/12 ml-40'>
          ...
          <div className='w-11/12 m-auto bg-white mt-12'>
            <DocItem onChange={setDocContent} initialContent={data.description} />
          </div>
        </div>
      </div>
    );
